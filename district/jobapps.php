        ...
        <div class="panel-body table-responsive">
            ...
            <table id="example0" class="table display">
                ...
                <tbody>
                    ...
                    $query=mysql_query("SELECT *,(SELECT jobs.title FROM jobs WHERE jobs.id=jobapps.job) AS jbs_title FROM jobapps WHERE jobapps.district='$di_inst'");
                    while ($row=mysql_fetch_array($query)) {
                    
                    ?>
                    <tr>
                        <td><?php $id=$row['id']; $applicant_name=$row['applicant_name']; echo $row['applicant_name'];?></td>
                        <td><?php echo $row['jbs_title'];?></td>
                        <td><a href="../assets/uploads/cv/<?php echo $row['applicant_cv'];?>">Download CV</a></td>
                        <td><?php $email=$row['applicant_email']; echo $row['applicant_email'];?></td>
                        <td><?php 
                            if($row['status']==0) {
                                echo "<a href='#judge-modal' data-toggle='modal' class='btn btn-xs btn-theme btn-default'>Not Yet Judged</a>";
                            }elseif($row['status']==1){
                                echo "Selected <span class='icon color7'><i class='fa fa-check'></i></span>" ;
                            }elseif($row['status']==2){
                ...
                <div class="modal-body">
                  <div class="form-group">
                    ...
                    <select class="form-control" name="mention" required>
                        ...
                        <option value="1">Selected <span class='icon color7'><i class='fa fa-check'></i></span></option>
